...
(define (get-procedures-list n F1 F2 F3)
    (map (λ (x) (cond
                  ((= x 1) (apply-function F1))
                  ((= x 2) (apply-function F2))
                  ((= x 3) (apply-function F3))))
...
(define (multiply-func M)
  (λ (x y z) (multiply M (list x y z))))

(define get-nth-ppt-from-matrix-transformations
  (λ (n) ((get-nth-tuple n (multiply-func T1) (multiply-func T2) (multiply-func T3)) '(3 4 5))))
...
(define get-nth-ppt-from-GH-quadruples
  (λ (n) (list
            (* (car (L n)) (cadddr (L n)))
            (* 2 (cadr (L n)) (caddr (L n)))
            (+ (expt (cadr (L n)) 2) (expt (caddr (L n)) 2)))))
